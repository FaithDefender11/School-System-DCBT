...
<?php 
    ...
    if(isset($_GET['id'])){
        ...
        if($_SERVER['REQUEST_METHOD'] === "POST"
            ...
            $wasSuccess = $enrollmentPayment->GetOptionsAndSinkToTheEnrollmentForm(
                $option, $enrollment_id, $downPayment, $cashierUserId);

            if($wasSuccess == true){

                # Update form.
                $wasSuccessUpdate = $enrollment->UpdatePaymentMethodAndStatusIntoPartial(
                    $enrollment_id, $optionCount, $current_school_year_id);

                if($wasSuccessUpdate){
                    Alert::success("Payment schedule has been linked and the enrollment form payment method is set to Partial.", $back_url);
                    exit();
                        ...
                        <div class="card-body">
                            <form method='POST'>
                                ...
                                <div class='form-group mb-2'>
                                    <label class='mb-2'>Installment plan</label>
                                ...
                                <div id="paymentPopulateTable"></div>

                                <div class="modal-footer">
                                    <button type='submit' class='btn btn-success'
                                    name='attach_schedule_payment_btn_<?php echo $enrollment_id; ?>'
                                    onclick='return confirm("Are you sure you want to attach the selected payment schedule ?");'>Attach to form</button>
                                </div>
