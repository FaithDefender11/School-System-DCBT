<?php 
    ...
    // $getEnrollmentRecords = $enrollment->getEnrollmentRecords($current_school_year_id);
    ...
    // var_dump($deActivatedElmsAccount);
          ...
          <table style="width: 100%" id="account_table" class="a">
              ...
              <tbody>
                    <?php
                        ...
                        foreach ($deActivatedElmsAccount as $key => $row) {
                            
                            $student_id = $row['student_id'];
                            $enrollment_id = $row['enrollment_id'];
                            $lms_student_id = $row['lms_student_id'];
                            ...
                            $date_deactivation = $row['date_deactivation'];
                            $date_deactivation = date("M d, Y h:i a", strtotime($date_deactivation));

                            $activateFromDeactivateAccount = "activateFromDeactivateAccount($enrollment_id, $lms_student_id)";

                            $button_url = "
                              <button class='default clean'
                                onclick='$activateFromDeactivateAccount'>
                                  Activate
                              </button>
                            ";
...
<script>
    ...
    function activateFromDeactivateAccount(enrollment_id, lms_student_id){

      var enrollment_id = parseInt(enrollment_id);
      var lms_student_id = parseInt(lms_student_id);
      
      Swal.fire({
          icon: 'question',
          title: `Are you sure you want to de-activate student LMS account?`,
          ...
          cancelButtonText: 'Cancel'
              ...
              if (result.isConfirmed) {
                  ...
                  $.ajax({
                      url: "../../ajax/pending/activateFromDeactivateAccount.php",
                      type: 'POST',
                      data: {
                          enrollment_id,
                          lms_student_id
