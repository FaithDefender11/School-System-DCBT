...
$sortBy = $columnNames[$columnIndex] ?? 'cashier_confirmation_date'; // Default to submission_creation column if the selected column is not found in the array
...
$stmt = $con->prepare("SELECT COUNT(*) AS allcount 
    ...
    WHERE  (t2.is_new_enrollee = :is_new_enrollee 
...
if ($row != null) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $cashier_confirmation_date = date("F d, Y g:i a", strtotime($cashier_confirmation_date));
