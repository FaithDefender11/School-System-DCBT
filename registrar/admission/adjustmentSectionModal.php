<?php 
    ?>
    <div class="modal fade" id="adjustmentSectionModalBtn" tabindex="-1"
        aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                
                <div class="modal-header">
                    <h4 class='modal-title text-center'>Waitinglist Section Adjustment</h4>
                  
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <form id="saveAdjustmentForm">
...
<script>
    $(document).on('submit', '#saveAdjustmentForm', function (e) {
            ...
            success: function (response) {
                ...
                if(output == "update_success_form_enrolled"){
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Success',
                        // text: 'Enrolled Form has been assigned to opened section.',
                        text: 'Enrollment form has been successfully moved.',
                        }).then((result) => {
                      
                        if (result.isConfirmed) {

                            $('#adjustmentSectionModalBtn').modal('hide');
                            $('#saveAdjustmentForm')[0].reset();

                            window.location.href = `../student/record_details.php?id=${student_id}&enrolled_subject=show`;
                        }
                    });
