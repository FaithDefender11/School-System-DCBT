<?php
    ...
    public function GetTermTimeFrame($term,
        ...
        if($query->rowCount() > 0){
            ...
            $start_enrollment_date = $start_enrollment_date_db !== NULL ? date('Y-m-d H:i a', strtotime($start_enrollment_date_db)) : 'Not Set';
            ...
            $end_enrollment_date = $end_enrollment_date_db !== NULL ? date('Y-m-d H:i a', strtotime($end_enrollment_date_db)) : $endEnrollmentEnded;
            // $final_exam_enddate = $final_exam_enddate_db !== NULL ? date('Y-m-d H:i a', strtotime($final_exam_enddate_db)) : $finalExamEnded;
            ...
            $class_startdate = $class_startdate !== NULL ? date('Y-m-d H:i a', strtotime($class_startdate)) : 'Not Set';


            $class_enddate_db = $row['class_enddate'];
            $class_enddate = $class_enddate_db !== NULL ? date('Y-m-d H:i a', strtotime($class_enddate_db)) : 'Not Set';
            $class_enddate_status = $this->CheckIfEnded($class_enddate_db);


            $prelim_exam_startdate = $row['prelim_exam_startdate'];
            $prelim_exam_startdate = $prelim_exam_startdate !== NULL ? date('Y-m-d H:i a', strtotime($prelim_exam_startdate)) : 'Not Set';

            $prelim_exam_enddate_db = $row['prelim_exam_enddate'];
            $prelim_exam_enddate = $prelim_exam_enddate_db !== NULL ? date('Y-m-d H:i a', strtotime($prelim_exam_enddate_db)) : 'Not Set';
            $prelim_exam_enddate_status = $this->CheckIfEnded($prelim_exam_enddate_db);


            $midterm_exam_startdate = $row['midterm_exam_startdate'];
            $midterm_exam_startdate = $midterm_exam_startdate !== NULL ? date('Y-m-d H:i a', strtotime($midterm_exam_startdate)) : 'Not Set';

            $midterm_exam_enddate_db = $row['midterm_exam_enddate'];
            $midterm_exam_enddate = $midterm_exam_enddate_db !== NULL ? date('Y-m-d H:i a', strtotime($midterm_exam_enddate_db)) : 'Not Set';
            $midterm_exam_enddate_status = $this->CheckIfEnded($midterm_exam_enddate_db);

            $prefinal_exam_startdate = $row['prefinal_exam_startdate'];
            $prefinal_exam_startdate = $prefinal_exam_startdate !== NULL ? date('Y-m-d H:i a', strtotime($prefinal_exam_startdate)) : 'Not Set';

            $prefinal_exam_enddate_db = $row['prefinal_exam_enddate'];
            $prefinal_exam_enddate = $prefinal_exam_enddate_db !== NULL ? date('Y-m-d H:i a', strtotime($prefinal_exam_enddate_db)) : 'Not Set';
            $prefinal_exam_enddate_status = $this->CheckIfEnded($prefinal_exam_enddate_db);

            $final_exam_startdate = $row['final_exam_startdate'];
            $final_exam_startdate = $final_exam_startdate !== NULL ? date('Y-m-d H:i a', strtotime($final_exam_startdate)) : 'Not Set';
            ...
            $final_exam_enddate_db = date('Y-m-d H:i a', strtotime($final_exam_enddate_db));
            ...
            //     ? date('Y-m-d H:i a', strtotime($final_exam_enddate_db)) 
            //     : $finalExamEnded;

            $final_exam_enddate_status = $this->CheckIfEnded($final_exam_enddate_db);

            $break_startdate = $row['break_startdate'];
            $break_startdate = $break_startdate !== NULL ? date('Y-m-d H:i a', strtotime($break_startdate)) : 'Not Set';

            $break_enddate_db = $row['break_enddate'];
            $break_enddate = $break_enddate_db !== NULL ? date('Y-m-d H:i a', strtotime($break_enddate_db)) : 'Not Set';
    ...
    public function SetAnotherRowOfSchoolYear($school_year_id){
        ...
        $now = date("Y-m-d H:i:s");
        $gettingNextRow = $this->con->prepare("SELECT * 
        ...
        if($gettingNextRow->rowCount() > 0){
            ...
            // $wasSet = $this->SetSchoolYearIdIntoActive($next_school_year_id);
    ...
    public function SetSchoolYearIdIntoActive($next_school_year_id){
        $update = $this->con->prepare("UPDATE school_year
